 ...
 * 全程关键步骤均打日志，绝不静默放弃。CEF 从未初始化时看门狗不做任何动作
 * （无 MCEF-Shutdown 线程可挂），直接收工并留日志。</p>
 */
...
public final class ExitWatchdog {
    ...
    private static void watch() {
        ...
        while (true) {
            ...
            try {
                ...
                System.out.println("[mcphone_browser] ExitWatchdog: interrupted while polling running, disarmed");
                return;
            }
        }
        System.out.println("[mcphone_browser] ExitWatchdog: running=false detected, game is exiting");
        ...
        for (int i = 0; i < 20 && !active; i++) {
            ...
            if (!active) {
                try {
                    ...
                    System.out.println("[mcphone_browser] ExitWatchdog: interrupted during CEF active probe, disarmed");
                    return;
                }
            }
        }
        System.out.println("[mcphone_browser] ExitWatchdog: CEF active determination = " + active
            + " (probes=" + probes + ")");
        if (!active) {
        ...
        try {
            System.out.println("[mcphone_browser] ExitWatchdog: force-closing our browser screens, grace period starting (" + GRACE_MS + " ms)");
            BrowserScreen.forceClose();
        } catch (Throwable t) {
            System.out.println("[mcphone_browser] ExitWatchdog: forceClose failed, continuing (" + t + ")");
        }
        try {
            Thread.sleep(GRACE_MS);
        } catch (InterruptedException e) {
            System.out.println("[mcphone_browser] ExitWatchdog: interrupted during grace period, continuing to scan");
        }
        System.out.println("[mcphone_browser] ExitWatchdog: grace period ended, entering scan loop (max " + (MAX_WAIT_MS / 1000) + " s)");
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (waited >= nextScanLog) {
                System.out.println("[mcphone_browser] ExitWatchdog: scan @" + (waited / 1000)
            ...
            if (!mainAlive && hung == null) {
                System.out.println("[mcphone_browser] ExitWatchdog: main thread gone and no CEF threads remain, JVM should exit naturally");
                return;
            }
            ...
            try {
                ...
                System.out.println("[mcphone_browser] ExitWatchdog: interrupted during scan loop, continuing");
            }
        }
        ...
        boolean mainAlive = isMainThreadAlive();
        String hung = hungCefThreads();
        System.out.println("[mcphone_browser] ExitWatchdog: scan deadline (60s) reached: main thread alive=" + mainAlive
            + ", hung CEF threads=" + (hung != null ? hung : "none"));
        if (mainAlive) {
            // 世界保存("Saving chunks")按 1.7.10 退出流程发生在 running=false 之前，
            // 此处主线程卡死只可能是清理挂起；dump 堆栈留证后无条件强杀。
            System.out.println("[mcphone_browser] ExitWatchdog: main thread still alive 60s after running=false —"
        ...
        System.out.println("[mcphone_browser] ExitWatchdog: main thread gone and no CEF threads remain at deadline, standing down");
    }
    ...
    private static void forceHalt() {
        System.out.println("[mcphone_browser] ExitWatchdog: forcing halt(0) via reflection to bypass FML's Runtime.halt -> System.exit redirect");
        try {
            Method halt = Runtime.class.getMethod("halt", int.class);
            halt.invoke(Runtime.getRuntime(), 0);
            // 正常情况下上一行不会返回（进程已死）
            System.out.println("[mcphone_browser] ExitWatchdog: reflective halt returned unexpectedly, process still alive");
        } catch (Throwable t) {
            System.out.println("[mcphone_browser] ExitWatchdog: reflective halt failed (" + t + "), falling back to FMLCommonHandler.exitJava(0, false)");
            try {
                FMLCommonHandler.instance().exitJava(0, false);
            } catch (Throwable t2) {
                System.out.println("[mcphone_browser] ExitWatchdog: FMLCommonHandler.exitJava failed (" + t2 + "), last resort: direct Runtime.halt (may be FML-redirected to System.exit)");
    ...
    private static void dumpMainThreadStack() {
        try {
            for (Map.Entry<Thread, StackTraceElement[]> e : Thread.getAllStackTraces().entrySet()) {
                ...
                sb.append("[mcphone_browser] ExitWatchdog: stack of thread \"").append(t.getName())
            ...
            System.out.println("[mcphone_browser] ExitWatchdog: main thread \"" + MAIN_THREAD_NAME + "\" not found in stack dump");
        } catch (Throwable t) {
            System.out.println("[mcphone_browser] ExitWatchdog: main thread stack dump failed (" + t + ")");
        }
    }
