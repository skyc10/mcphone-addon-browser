 ...
 *
 * <p>时序（仅当 CEF 已初始化时介入；从未初始化则直接收工让 JVM 自然退出）：
 * running=false → 异步关掉我们打开的浏览器（close 的 native 调用会阻塞，
 * 上一版同步执行把看门狗自己卡死了）→ 宽限 5 秒 → 扫描循环：</p>
 * <ul>
...
public final class ExitWatchdog {
    ...
    public static void arm() {
        try {
            ...
            logs.mkdirs();
            logFile = new File(logs, "mcphone_browser_watchdog.log");
        } catch (Throwable t) {
            logFile = null;
        }
    ...
    private static void log(String msg) {
        String line = "[mcphone_browser] ExitWatchdog: " + msg;
        if (!fileOnly) {
        ...
        for (int i = 0; i < 20 && !active; i++) {
            ...
            if (!active) {
                try {
                    ...
                    log("interrupted during CEF active probe, disarmed");
                    return;
                }
            }
        }
        log("CEF active determination = " + active);
        if (!active) {
            log("CEF was never initialized, standing down");
            return;
        }
