...
- **键盘注入无 keyCode（仅旧 MCEF 内核）**：MCEF 0.6/0.7 的 `injectKeyXxx(char, modifiers)` 无法表达功能键（F1–F12、方向键、组合键），网页内这类按键可能无效；modern 内核（v1.0.2-modern.3 起）已通过 `injectKeyXxxByKeyCode` 管道支持方向键/Delete/Home/End/PgUp/PgDn（F1–F12 仍未映射）
